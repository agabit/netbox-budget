...
# ItemCode views
class ItemCodeListView(generic.ObjectListView):
    queryset = models.ItemCode.objects.all()
    table = tables.ItemCodeTable
    filterset = ItemCodeFilterSet
    filterset_form = forms.ItemCodeFilterForm

class ItemCodeView(generic.ObjectView):
    queryset = models.ItemCode.objects.all()

class ItemCodeEditView(generic.ObjectEditView):
    queryset = models.ItemCode.objects.all()
    form = forms.ItemCodeForm

class ItemCodeDeleteView(generic.ObjectDeleteView):
    queryset = models.ItemCode.objects.all()


# BudgetPlan views
class BudgetPlanListView(generic.ObjectListView):
    queryset = models.BudgetPlan.objects.prefetch_related("supplier", "contract", "item_code")
...
class BudgetPlanView(generic.ObjectView):
    queryset = models.BudgetPlan.objects.prefetch_related(
        "supplier", "contract", "tenders", "item_code"
    )
